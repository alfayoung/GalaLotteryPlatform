<template>
  ...
  import {useuserStore} from "/src/stores/user.js"
    export default {
      ...
      data() {
        return {
          timer: this.vRoll, // 球体转动速率
          radius: 0, // 词云球体面积大小
          dtr: this.vAngle, //鼠标滑过球体转动速度,被我改成旋转方向转动速率
          active: false, // 默认加载是否开启转动
          lasta: 10, // 上下转动
          lastb: 10, // 左右转动
          distr: true,
          tspeed: 30, // 鼠标移动上去时球体转动
          mouseX: 0,
          mouseY: 0,
          tagAttrList: [],
          tagContent: null,
          ...
          sinC: '',
          cosC: '',
          dataList: useuserStore().userList
        }
      },
      mounted () {
        this.$nextTick(() => {
      ...
      methods:{
        restartRotation() {
          // 停止当前的旋转
          clearInterval(this.initWordCloud);
          // 启动一个新的旋转
          this.initWordCloud = setInterval(this.update, this.timer);
        },
        // 获取点击文本信息
        getDataInfo (item) {
          console.log(item, 'item')
          console.log(this.vRoll,this.timer)
        },
        initWordCloud () {
        ...
        update () {
          ...
          this.doPosition();
          this.depthSort();
        let cos = Math.cos(this.dtr)
        let sin = Math.sin(this.dtr)
        let _a = this.lasta * cos - this.lastb * sin 
		    let _b = this.lasta * sin + this.lastb * cos
		    this.lasta = _a
		    this.lastb = _b
        },
        doPosition() {
